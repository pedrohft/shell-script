...
RetornaFormato () {
# A função retorn 0 para data em formato BR (DDMMYYYY),
# 1 para formato US (MMDDYYYY) e 2 quando é impossível determinar.
# Quando dia = mês, considera-se formato BR.
# Retorna 3 para formato inválido
    if [ $VAL1 -le 12 -a $VAL1 -eq $VAL2 ] # Dia = Mes = Formato BR
    then
        return 0
    elif [ $VAL1 -gt 12 -a $VAL1 -le $VAL2 ] # Formato BR
    then
        return 0
    elif [ $VAL1 -le 12 -a $VAL1 -gt $VAL2 ] # Formato US
    then 
        return 1
    elif [ $VAL1 -le 12 -a $VAL1 -le $VAL2 ] # Formato Indefinido, mas válido
    then
        return 2
    fi
}
...
InverteDiaMes () {
# Função que inverte os valores do dia com mês e retorn o valor
    RetornaFormato $1
    local FORMATO=$?
    if [ $FORMATO -ne 3 ]
    then    
...
IncluiBarra () {
# Função que inclui barras na data 
    RetornaFormato $1
    local FORMATO=$?
    if [ $FORMATO -ne 3 ]
    then    
...
DataExtenso () {
# Função que recebe uma data em formato BR e exibe a data no formato extenso
    RetornaFormato $1
    local RETURN_FORM=$?
    case $RETURN_FORM in
...
case $1 in
    ...
    "--help")
        ...
        echo "-i = Inverte formato BR para US e US para BR. Inlui as Barras"
